...
// Simple built-in OTA updater for headless Generic ESP32 builds.
namespace {
  ...
  const char OTA_SSID[] = "MarauderOTA";
  const char OTA_PASSWORD[] = "justcallmekoko";
...
<title>Marauder OTA</title>
<style>
body{font-family:Arial,sans-serif;background:#111;color:#eee;margin:0;padding:24px;text-align:center}
.box{max-width:520px;margin:auto;background:#1d1d1d;padding:24px;border-radius:14px}
h2{margin-top:0} input{width:100%;margin:12px 0;padding:12px;box-sizing:border-box}
button{padding:12px 20px;border:0;border-radius:8px;font-weight:bold;cursor:pointer}
#status{margin-top:16px;word-break:break-word}
</style></head><body>
<div class="box"><h2>ESP32 Marauder OTA</h2>
<p>Selecione o firmware <b>.bin</b> e envie para atualizar.</p>
<form id="f">
<input id="file" type="file" accept=".bin,application/octet-stream" required>
<button type="submit">ATUALIZAR</button></form>
<div id="status"></div></div>
<script>
document.getElementById('f').onsubmit=function(e){
 e.preventDefault();var f=document.getElementById('file').files[0],s=document.getElementById('status');
 if(!f){s.textContent='Selecione um arquivo .bin.';return;}
 s.textContent='Enviando...';var x=new XMLHttpRequest();x.open('POST','/update',true);
 x.upload.onprogress=function(e){if(e.lengthComputable)s.textContent='Enviando: '+Math.round(e.loaded/e.total*100)+'%';};
 x.onload=function(){s.innerHTML=x.status==200?'Atualização concluída. Reiniciando...':'Falha: '+x.responseText;};
 x.onerror=function(){s.textContent='Erro de conexão durante o envio.';};
 x.send(f);
};
</script></body></html>
)rawliteral";
  ...
  bool otaAuth() {
    if (otaServer.authenticate(OTA_USER, OTA_PASS)) return true;
  ...
  void otaHandleRoot() {
    if (!otaAuth()) return;
    otaServer.send(200, "text/html; charset=utf-8", OTA_PAGE);
  }
  ...
  void otaHandleUpload() {
    ...
    if (upload.status == UPLOAD_FILE_START) {
      ...
      // Checa autenticação ANTES de aceitar qualquer byte do firmware.
      // otaServer.authenticate() só valida, não manda resposta HTTP,
      // então é seguro chamar aqui em pleno meio do upload.
      otaUnauthorized = !otaServer.authenticate(OTA_USER, OTA_PASS);
      if (otaUnauthorized) {
        Serial.println(F("[OTA] Upload rejeitado: credenciais invalidas"));
        return;
      }
  ...
  void otaHandleUpdate() {
    ...
    if (otaUnauthorized) {
      otaServer.send(401, "text/plain; charset=utf-8", "Nao autorizado.");
      return;
    }

    if (otaUploadOk) {
      otaServer.send(200, "text/plain; charset=utf-8", "OK - reiniciando...");
      delay(1000);
      ESP.restart();
    } else {
      otaServer.send(500, "text/plain; charset=utf-8", "Falha ao gravar o firmware.");
    }
  }

  void startMarauderOTA() {
    if (otaRunning) return;

    otaUploadOk = false;
    otaUnauthorized = false;
    ...
    Serial.println(F("[OTA] Wi-Fi: MarauderOTA"));
    Serial.println(F("[OTA] Senha: justcallmekoko"));
    Serial.println(F("[OTA] Login: admin / admin"));
    Serial.print(F("[OTA] Acesse: http://"));
    Serial.println(WiFi.softAPIP());
  }

  void handleMarauderOTA() {
    if (otaRunning)
