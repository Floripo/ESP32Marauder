...
// Uses the same ESPAsyncWebServer already used by Marauder, avoiding
// a WebServer/ESPAsyncWebServer HTTP_METHOD name collision.
namespace {
...
document.getElementById('f').onsubmit=function(e){
 e.preventDefault();
 var f=document.getElementById('file').files[0],s=document.getElementById('status');
 ...
 x.upload.onprogress=function(e){
   if(e.lengthComputable)s.textContent='Enviando: '+Math.round(e.loaded/e.total*100)+'%';
 };
 x.onload=function(){
   s.textContent=x.status==200?'Atualização concluída. Reiniciando...':'Falha: '+x.responseText;
 };
 ...
 x.send(fd);
  ...
  bool otaAuth(AsyncWebServerRequest *request) {
    return request->authenticate(OTA_USER, OTA_PASS);
  }

  void otaHandleRoot(AsyncWebServerRequest *request) {
    if (!otaAuth(request)) {
      request->requestAuthentication();
      return;
    }
    request->send(200, "text/html; charset=utf-8", OTA_PAGE);
  }
  ...
  void otaHandleUpload(
    ...
    if (index == 0) {
      ...
      otaUnauthorized = !otaAuth(request);
    ...
    if (otaUnauthorized || otaUploadFailed)
      return;
  ...
  void otaHandleUpdate(AsyncWebServerRequest *request) {
    if (!otaAuth(request)) {
      request->send(401, "text/plain; charset=utf-8", "Nao autorizado.");
      return;
    }
    ...
    if (otaUploadOk && !otaUploadFailed) {
      request->send(200, "text/plain; charset=utf-8", "OK - reiniciando...");
      delay(1000);
      ESP.restart();
      return;
    }

    request->send(500, "text/plain; charset=utf-8", "Falha ao gravar o firmware.");
  }
